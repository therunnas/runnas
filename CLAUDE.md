...
### Three content systems — add content by dropping files, not editing registries
...
3. **Projects** — an Astro **data collection** (`type: "data"`). Drop a `.yaml` file in [src/content/projects/](src/content/projects/); the schema is in [src/content/config.ts](src/content/config.ts) (`title`, `description`, `tags[]`, optional `sourceUrl`/`liveUrl`, and `order`). [src/pages/index.astro](src/pages/index.astro) reads them with `getCollection("projects")` and sorts by `order` (asc). The generated `astro:content` types live in `.astro/` (gitignored, regenerated by `astro sync`/dev/build) — don't commit them.
